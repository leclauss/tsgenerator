...
def MASS(time_series, query):
  ...
  ts_df = pd.DataFrame({"ts": time_series})
  time_series_mean = ts_df["ts"].rolling(m).mean()             #Time series rolling mean; window is the length of the query
  time_series_std = ts_df["ts"].rolling(m).std(ddof=0)        #Time series rolling standard deviation; window is the length of the query. No degrees of freedom correction.
...
def roll_mean_std(ts, ws):
  ...
  for ww in range(min(len(ts), ws)):
    sum += ts[ww];
    squareSum += ts[ww] * ts[ww];

  means.append(sum * rws)
  buf = squareSum * rws - means[0] * means[0]
  if buf > 0:
    stds.append(np.sqrt(buf))
  else:
    stds.append(0)

  for w in range(1,len(ts) - ws + 1):
    sum += ts[w + ws - 1] - ts[w - 1]
    means.append(sum * rws)
    squareSum += ts[w + ws - 1] * ts[w + ws - 1] - ts[w - 1] * ts[w - 1]
    buf = squareSum * rws - means[w] * means[w]
    if buf > 0:
      stds.append(np.sqrt(buf))
    else:
      0
...
def sim(sub0, sub1, ts, means, stds, ws, r):
  #calculate means
  meanPos0 = means[sub0];
  meanPos1 = means[sub1];
  ...
  for i in range(ws):
    normPos0.append((ts[sub0 + i] - meanPos0) / stdPos0)
    normPos1.append((ts[sub1 + i] - meanPos1) / stdPos1)
...
def print_nsm_sub(sub, ts, means, stds, ws, r):
  print(str(sub))
  i = 0
  while i < len(ts) - ws + 1:
    if ws <= abs(sub - i) and sim(sub, i, ts, means, stds, ws, r) <= r:
      print(str(i))
      i = i + 2*ws
    else:
      i = i + 1
...
def main():
  if len(sys.argv) == 4:
    ...
    if my_file.is_file():
      target_series = np.genfromtxt(sys.argv[1], delimiter='\n')
      
      mp = np.round(STAMP(target_series=target_series, subsequence_length=int(sys.argv[2]), max_time=10000, parallel=False), 6)
      x = np.argmin(mp.Matrix_Profile)
      y = mp.Matrix_Profile_Index[x]
      means, stds = roll_mean_std(target_series, int(sys.argv[2]))
      print_nsm_sub(x, target_series, means, stds, int(sys.argv[2]), float(sys.argv[3]))
      print("next")
      print_nsm_sub(int(y), target_series, means, stds, int(sys.argv[2]), float(sys.argv[3]))
      print("end")
      #print("top motif pair ( " + str(x) + ", " + str(int(y)) + " ) distance is " + str(mp.Matrix_Profile[x]))
      #print(str(x) + "\n" + str(int(y)) + "\nend\n")
    else:
