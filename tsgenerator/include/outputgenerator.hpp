...
protected:
  ...
  std::ofstream outputFileMotifSets;

  ///\brief Stores the GNUPlot script output file pointer.
  ///
  ///This variable stores the pointer pointing at the output file containing
  ///the GNUPlot script.
  std::ofstream outputFileGNUPlotScript;
  ...
  std::string outputFileMotifSetsName;
  ...
  std::string outputFileGNUPlotScriptName;
...
public:
  ...
  ///name with default file name "time_series". It chooses another directory
  ///path, if a directory with the same name already exists.
  OutputGenerator(const std::string &outputFileName_in);
  ...
  void printTimeSeriesHorizontal(const std::vector<double> &timeSeries_in,
      const std::vector<double> &d_in, const std::vector<int> &windowSizes_in,
      const std::vector<std::vector<int>> &motifSetPositions_in);
  ...
  void printTimeSeriesVertical(const std::vector<double> &timeSeries_in, const
      std::vector<double> &d_in, const std::vector<int> &windowSizes_in, const
      std::vector<std::vector<int>> &motifSetPositions_in);
  ...
  void setTimeSeriesName(const std::string &timeSeriesName_in);
};
