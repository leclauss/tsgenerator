...
namespace tsg {
  ...
  void scrimpPP(const rseq &timeSeries_in, const rseq &ASum_in, const rseq
      ...
      double step_in)
  {
    auto g = std::default_random_engine(std::random_device().entropy()
      ? std::random_device()()
      : (unsigned
    ...
    int step = (int)(floor(step_in*window));

    if (step_in < 0.0)
      step = (int)(floor(0.4*window));
    ...
    for (int i=0; i<ProfileLength; i++)
    {
      profile.push_back(std::numeric_limits<double>::infinity());
    ...
    //Sample subsequences with a fixed step, then random shuffle their
    //computation order
    iseq idx;
    for (int i = 0; i < timeSeriesLength - window + 1; i += step)
      idx.push_back(i);
    ...
    for (int idx_i = 0; idx_i < (int) idx.size(); idx_i++)
    {
      ...
      double lastzz = lastz;
      for (int k = 1; k < step && i + k < timeSeriesLength - window
      ...
      lastz = lastzz;
      for (int k = 1; k < step && i - k >= 0 && j - k >= 0; k++)
      {
    ...
    rseq dotproduct(timeSeriesLength);
